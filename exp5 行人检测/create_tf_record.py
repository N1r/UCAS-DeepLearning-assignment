...
#  print('height = ',height)
  ...
  poses = []
  difficult_obj = []
#  f1 = open(os.path.join('jl',str(data['filename']) + '.txt'), 'w+')
  for obj in data['object']:
    difficult_obj.append(int(0))
    xmin.append(float(obj['bndbox']['xmin']) / width)
#    f1.write(str(float(obj['bndbox']['xmin']) / width)+'\t')
    ymin.append(float(obj['bndbox']['ymin']) / height)
#    f1.write(str(float(obj['bndbox']['ymin']) / height)+'\t')
    xmax.append(float(obj['bndbox']['xmax']) / width)
#    f1.write(str(float(obj['bndbox']['xmax']) / width)+'\t')
    ymax.append(float(obj['bndbox']['ymax']) / height)
#    f1.write(str(float(obj['bndbox']['ymax']) / height)+'\n')
...
def main(_):
  label_map_dict = label_map_util.get_label_map_dict('test_annotations/label_map.pbtxt')
